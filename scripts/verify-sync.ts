...
async function main() {
  ...
  const [fotosKeys, thumbsFotosKeys, videosKeys, thumbsVideosKeys] =
    await Promise.all([
  ...
  // 2. Contar DB
  const dbFotosResult = await pool.query(
    'SELECT COUNT(*) FROM "Photo" WHERE "userId" = $1 AND "largeS3Key" IS NOT NULL',
    [USER_ID],
  );
  const dbVideosResult = await pool.query(
    'SELECT COUNT(*) FROM "Photo" WHERE "userId" = $1 AND "largeS3Key" IS NULL',
    [USER_ID],
  );
  ...
  const dbAll = await pool.query(
    'SELECT "s3Key", "thumbS3Key", "largeS3Key" FROM "Photo" WHERE "userId" = $1',
    [USER_ID],
  );
  ...
  console.log(
    `  Huérfanos: ${orphanVideos.length + orphanThumbsVideos.length}`,
  );

  const allOk =
    dbFotos === r2FotosCount &&
